...
namespace EmberEngine
{
	...
	float MathF_FMA::Vector2SqMagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	double MathF_FMA::Vector2MagnitudeImpl(double* vec)
	{
	...
	double MathF_FMA::Vector2SqMagnitudeImpl(double* vec)
	{
		__m128d vecReg = _mm_loadu_pd(vec);
		__m128d sqReg = _mm_mul_pd(vecReg, vecReg);
	...
	float MathF_FMA::Vector2MagnitudeImpl(int* vec)
	{
	...
	float MathF_FMA::Vector2SqMagnitudeImpl(int* vec)
	{
	...
	void MathF_FMA::Vector2NormaliseImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	void MathF_FMA::Vector2NormaliseImpl(double* vec)
	{
		__m128d vecReg = _mm_loadu_pd(vec);
		__m128d sqReg = _mm_mul_pd(vecReg, vecReg);
	...
	void MathF_FMA::ResetVector3Impl(float* vec)
	{
		_mm_storeu_ps(vec, _mm_setzero_ps());
	}

	void MathF_FMA::ResetVector3Impl(double* vec)
	{
		_mm256_storeu_pd(vec, _mm256_setzero_pd());
	}

	void MathF_FMA::ResetVector3Impl(int* vec)
	{
		_mm_storeu_si128((__m128i*)vec, _mm_setzero_si128());
	}

	void MathF_FMA::Vector3AddImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_add_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector3AddImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_add_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector3AddImpl(int* dst, int* vec1, int* vec2)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_add_epi32(_mm_loadu_si128((__m128i*)vec1), _mm_loadu_si128((__m128i*)vec2)));
	}

	void MathF_FMA::Vector3SubImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_sub_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector3SubImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_sub_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector3SubImpl(int* dst, int* vec1, int* vec2)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_sub_epi32(_mm_loadu_si128((__m128i*)vec1), _mm_loadu_si128((__m128i*)vec2)));
	}

	void MathF_FMA::Vector3MulImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_mul_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector3MulImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_mul_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector3MulImpl(int* dst, int* vec1, int* vec2)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_mullo_epi32(_mm_loadu_si128((__m128i*)vec1), _mm_loadu_si128((__m128i*)vec2)));
	}

	void MathF_FMA::Vector3MulImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_mul_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_FMA::Vector3MulImpl(double* dst, double* vec, double scalar)
	{
		_mm256_storeu_pd(dst, _mm256_mul_pd(_mm256_loadu_pd(vec), _mm256_set1_pd(scalar)));
	}

	void MathF_FMA::Vector3MulImpl(int* dst, int* vec, int scalar)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_mullo_epi32(_mm_loadu_si128((__m128i*)vec), _mm_set1_epi32(scalar)));
	}

	void MathF_FMA::Vector3DivImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_div_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector3DivImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_div_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector3DivImpl(int* dst, int* vec1, int* vec2)
	{
		dst[0] = vec1[0] / vec2[0];
		dst[1] = vec1[1] / vec2[1];
		dst[2] = vec1[2] / vec2[2];
	...
	void MathF_FMA::Vector3DivImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_div_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_FMA::Vector3DivImpl(double* dst, double* vec, double scalar)
	{
		_mm256_storeu_pd(dst, _mm256_div_pd(_mm256_loadu_pd(vec), _mm256_set1_pd(scalar)));
	}

	void MathF_FMA::Vector3DivImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] / scalar;
		dst[1] = vec[1] / scalar;
		dst[2] = vec[2] / scalar;
	...
	float MathF_FMA::Vector3MagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	float MathF_FMA::Vector3SqMagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	double MathF_FMA::Vector3MagnitudeImpl(double* vec)
	{
	...
	double MathF_FMA::Vector3SqMagnitudeImpl(double* vec)
	{
		__m256d vecReg = _mm256_loadu_pd(vec);
		__m256d sqReg = _mm256_mul_pd(vecReg, vecReg);
	...
	float MathF_FMA::Vector3MagnitudeImpl(int* vec)
	{
	...
	float MathF_FMA::Vector3SqMagnitudeImpl(int* vec)
	{
	...
	void MathF_FMA::Vector3NormaliseImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	void MathF_FMA::Vector3NormaliseImpl(double* vec)
	{
		__m256d vecReg = _mm256_loadu_pd(vec);
		__m256d sqReg = _mm256_mul_pd(vecReg, vecReg);
	...
	void MathF_FMA::ResetVector4Impl(float* vec)
	{
		_mm_storeu_ps(vec, _mm_setzero_ps());
	}

	void MathF_FMA::ResetVector4Impl(double* vec)
	{
		_mm256_storeu_pd(vec, _mm256_setzero_pd());
	}

	void MathF_FMA::ResetVector4Impl(int* vec)
	{
		_mm_storeu_si128((__m128i*)vec, _mm_setzero_si128());
	}

	void MathF_FMA::Vector4AddImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_add_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector4AddImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_add_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector4AddImpl(int* dst, int* vec1, int* vec2)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_add_epi32(_mm_loadu_si128((__m128i*)vec1), _mm_loadu_si128((__m128i*)vec2)));
	}

	void MathF_FMA::Vector4SubImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_sub_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector4SubImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_sub_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector4SubImpl(int* dst, int* vec1, int* vec2)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_sub_epi32(_mm_loadu_si128((__m128i*)vec1), _mm_loadu_si128((__m128i*)vec2)));
	}

	void MathF_FMA::Vector4MulImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_mul_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector4MulImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_mul_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector4MulImpl(int* dst, int* vec1, int* vec2)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_mullo_epi32(_mm_loadu_si128((__m128i*)vec1), _mm_loadu_si128((__m128i*)vec2)));
	}

	void MathF_FMA::Vector4MulImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_mul_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_FMA::Vector4MulImpl(double* dst, double* vec, double scalar)
	{
		dst[0] = vec[0] * scalar;
		dst[1] = vec[1] * scalar;
		dst[2] = vec[2] * scalar;
	...
	void MathF_FMA::Vector4MulImpl(int* dst, int* vec, int scalar)
	{
		_mm_storeu_si128((__m128i*)dst, _mm_mullo_epi32(_mm_loadu_si128((__m128i*)vec), _mm_set1_epi32(scalar)));
	}

	void MathF_FMA::Vector4DivImpl(float* dst, float* vec1, float* vec2)
	{
		_mm_storeu_ps(dst, _mm_div_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2)));
	}

	void MathF_FMA::Vector4DivImpl(double* dst, double* vec1, double* vec2)
	{
		_mm256_storeu_pd(dst, _mm256_div_pd(_mm256_loadu_pd(vec1), _mm256_loadu_pd(vec2)));
	}

	void MathF_FMA::Vector4DivImpl(int* dst, int* vec1, int* vec2)
	{
		dst[0] = vec1[0] / vec2[0];
		dst[1] = vec1[1] / vec2[1];
		dst[2] = vec1[2] / vec2[2];
	...
	void MathF_FMA::Vector4DivImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_div_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_FMA::Vector4DivImpl(double* dst, double* vec, double scalar)
	{
		_mm256_storeu_pd(dst, _mm256_div_pd(_mm256_loadu_pd(vec), _mm256_set1_pd(scalar)));
	}

	void MathF_FMA::Vector4DivImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] / scalar;
		dst[1] = vec[1] / scalar;
		dst[2] = vec[2] / scalar;
	...
	float MathF_FMA::Vector4MagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
		return _mm_sqrt_ps(_mm_hadd_ps(_mm_hadd_ps(sqReg, sqReg), _mm_setzero_ps())).m128_f32[0];
	...
	float MathF_FMA::Vector4SqMagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
		return _mm_hadd_ps(_mm_hadd_ps(sqReg, sqReg), _mm_setzero_ps()).m128_f32[0];
	...
	double MathF_FMA::Vector4MagnitudeImpl(double* vec)
	{
		__m256d vecReg = _mm256_loadu_pd(vec);
		__m256d sqReg = _mm256_mul_pd(vecReg, vecReg);
		return _mm256_sqrt_pd(_mm256_hadd_pd(_mm256_hadd_pd(sqReg, sqReg), _mm256_setzero_pd())).m256d_f64[0];
	...
	double MathF_FMA::Vector4SqMagnitudeImpl(double* vec)
	{
		__m256d vecReg = _mm256_loadu_pd(vec);
		__m256d sqReg = _mm256_mul_pd(vecReg, vecReg);
		return _mm256_hadd_pd(_mm256_hadd_pd(sqReg, sqReg), _mm256_setzero_pd()).m256d_f64[0];
	...
	float MathF_FMA::Vector4MagnitudeImpl(int* vec)
	{
		return std::sqrt((float)_mm_hadd_epi32(_mm_hadd_epi32(_mm_mullo_epi32(_mm_loadu_si128((__m128i*)vec), _mm_loadu_si128((__m128i*)vec)), _mm_setzero_si128()), _mm_setzero_si128()).m128i_i32[0]);
	}

	float MathF_FMA::Vector4SqMagnitudeImpl(int* vec)
	{
		return (float)_mm_hadd_epi32(_mm_hadd_epi32(_mm_mullo_epi32(_mm_loadu_si128((__m128i*)vec), _mm_loadu_si128((__m128i*)vec)), _mm_setzero_si128()), _mm_setzero_si128()).m128i_i32[0];
	}

	void MathF_FMA::Vector4NormaliseImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
		_mm_storeu_ps(vec, _mm_div_ps(vecReg, _mm_set1_ps(_mm_sqrt_ps(_mm_hadd_ps(_mm_hadd_ps(sqReg, sqReg), _mm_setzero_ps())).m128_f32[0])));
	...
	void MathF_FMA::Vector4NormaliseImpl(double* vec)
	{
		__m256d vecReg = _mm256_loadu_pd(vec);
		__m256d sqReg = _mm256_mul_pd(vecReg, vecReg);
		_mm256_storeu_pd(vec, _mm256_div_pd(vecReg, _mm256_set1_pd(_mm256_sqrt_pd(_mm256_hadd_pd(_mm256_hadd_pd(sqReg, sqReg), _mm256_setzero_pd())).m256d_f64[0])));
	...
	void MathF_FMA::ResetMatrix4x4Impl(float* matrix)
	{
		_mm256_storeu_ps(matrix, _mm256_setzero_ps());
		_mm256_storeu_ps(matrix + 8, _mm256_setzero_ps());
	}

	void MathF_FMA::SetToIdentityImpl(float* matrix)
	{
		_mm256_storeu_ps(matrix, _mm256_setzero_ps());
		_mm256_storeu_ps(matrix + 8, _mm256_setzero_ps());
		matrix[0] = 1.0f;
	...
	void MathF_FMA::MultiplyMatrix4x4Impl(float* dst, float* matrix1, float* matrix2)
	{
	...
	void MathF_FMA::SetToTranslationImpl(float* matrix, float* vec2)
	{
		_mm256_storeu_ps(matrix, _mm256_setzero_ps());
		_mm256_storeu_ps(matrix + 8, _mm256_setzero_ps());
		matrix[0] = 1.0f;
	...
	void MathF_FMA::SetToScaleImpl(float* matrix, float* vec2)
	{
		_mm256_storeu_ps(matrix, _mm256_setzero_ps());
		_mm256_storeu_ps(matrix + 8, _mm256_setzero_ps());
		matrix[0] = vec2[0];
	...
	void MathF_FMA::SetToRotationImpl(float* matrix, float radians)
	{
		_mm256_storeu_ps(matrix, _mm256_setzero_ps());
		_mm256_storeu_ps(matrix + 8, _mm256_setzero_ps());
	...
	void MathF_FMA::SetToTranslationWithoutResetImpl(float* matrix, float* vec2)
	{
		matrix[0] = 1.0f;
		matrix[5] = 1.0f;
		matrix[10] = 1.0f;
	...
	void MathF_FMA::SetToScaleWithoutResetImpl(float* matrix, float* vec2)
	{
		matrix[0] = vec2[0];
		matrix[5] = vec2[1];
		matrix[10] = 1.0f;
	...
	void MathF_FMA::SetToRotationWithoutResetImpl(float* matrix, float radians)
	{
		float sinAngle = std::sin(radians);
		float cosAngle = std::cos(radians);
		matrix[0] = cosAngle;
