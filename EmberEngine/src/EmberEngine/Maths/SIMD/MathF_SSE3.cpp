...
namespace EmberEngine
{
	...
	void MathF_SSE3::Vector2MulImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_mul_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_SSE3::Vector2MulImpl(double* dst, double* vec, double scalar)
	{
		_mm_storeu_pd(dst, _mm_mul_pd(_mm_loadu_pd(vec), _mm_set1_pd(scalar)));
	}

	void MathF_SSE3::Vector2MulImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] * scalar;
		dst[1] = vec[1] * scalar;
	}
	...
	void MathF_SSE3::Vector2DivImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_div_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_SSE3::Vector2DivImpl(double* dst, double* vec, double scalar)
	{
		_mm_storeu_pd(dst, _mm_div_pd(_mm_loadu_pd(vec), _mm_set1_pd(scalar)));
	}

	void MathF_SSE3::Vector2DivImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] / scalar;
		dst[1] = vec[1] / scalar;
	}

	float MathF_SSE3::Vector2MagnitudeImpl(float* vec)
	{
	...
	float MathF_SSE3::Vector2SqMagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	double MathF_SSE3::Vector2MagnitudeImpl(double* vec)
	{
	...
	double MathF_SSE3::Vector2SqMagnitudeImpl(double* vec)
	{
		__m128d vecReg = _mm_loadu_pd(vec);
		__m128d sqReg = _mm_mul_pd(vecReg, vecReg);
	...
	float MathF_SSE3::Vector2MagnitudeImpl(int* vec)
	{
		int xSq = vec[0] * vec[0];
		int ySq = vec[1] * vec[1];
	...
	float MathF_SSE3::Vector2SqMagnitudeImpl(int* vec)
	{
		int xSq = vec[0] * vec[0];
		int ySq = vec[1] * vec[1];
	...
	void MathF_SSE3::Vector2NormaliseImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	void MathF_SSE3::Vector2NormaliseImpl(double* vec)
	{
		__m128d vecReg = _mm_loadu_pd(vec);
		__m128d sqReg = _mm_mul_pd(vecReg, vecReg);
	...
	void MathF_SSE3::Vector3MulImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_mul_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_SSE3::Vector3MulImpl(double* dst, double* vec, double scalar)
	{
		dst[0] = vec[0] * scalar;
		dst[1] = vec[1] * scalar;
		dst[2] = vec[2] * scalar;
	...
	void MathF_SSE3::Vector3MulImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] * scalar;
		dst[1] = vec[1] * scalar;
		dst[2] = vec[2] * scalar;
	...
	void MathF_SSE3::Vector3DivImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_div_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_SSE3::Vector3DivImpl(double* dst, double* vec, double scalar)
	{
		dst[0] = vec[0] / scalar;
		dst[1] = vec[1] / scalar;
		dst[2] = vec[2] / scalar;
	...
	void MathF_SSE3::Vector3DivImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] / scalar;
		dst[1] = vec[1] / scalar;
		dst[2] = vec[2] / scalar;
	...
	float MathF_SSE3::Vector3MagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	float MathF_SSE3::Vector3SqMagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	double MathF_SSE3::Vector3MagnitudeImpl(double* vec)
	{
	...
	double MathF_SSE3::Vector3SqMagnitudeImpl(double* vec)
	{
		double xSq = vec[0] * vec[0];
		double ySq = vec[1] * vec[1];
		double zSq = vec[2] * vec[2];
	...
	float MathF_SSE3::Vector3MagnitudeImpl(int* vec)
	{
		int xSq = vec[0] * vec[0];
		int ySq = vec[1] * vec[1];
		int zSq = vec[2] * vec[2];
	...
	float MathF_SSE3::Vector3SqMagnitudeImpl(int* vec)
	{
		int xSq = vec[0] * vec[0];
		int ySq = vec[1] * vec[1];
		int zSq = vec[2] * vec[2];
	...
	void MathF_SSE3::Vector3NormaliseImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
	...
	void MathF_SSE3::Vector3NormaliseImpl(double* vec)
	{
		double xSq = vec[0] * vec[0];
		double ySq = vec[1] * vec[1];
		double zSq = vec[2] * vec[2];
	...
	void MathF_SSE3::Vector4MulImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_mul_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_SSE3::Vector4MulImpl(double* dst, double* vec, double scalar)
	{
		dst[0] = vec[0] * scalar;
		dst[1] = vec[1] * scalar;
		dst[2] = vec[2] * scalar;
	...
	void MathF_SSE3::Vector4MulImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] * scalar;
		dst[1] = vec[1] * scalar;
		dst[2] = vec[2] * scalar;
	...
	void MathF_SSE3::Vector4DivImpl(float* dst, float* vec, float scalar)
	{
		_mm_storeu_ps(dst, _mm_div_ps(_mm_loadu_ps(vec), _mm_set1_ps(scalar)));
	}

	void MathF_SSE3::Vector4DivImpl(double* dst, double* vec, double scalar)
	{
		dst[0] = vec[0] / scalar;
		dst[1] = vec[1] / scalar;
		dst[2] = vec[2] / scalar;
	...
	void MathF_SSE3::Vector4DivImpl(int* dst, int* vec, int scalar)
	{
		dst[0] = vec[0] / scalar;
		dst[1] = vec[1] / scalar;
		dst[2] = vec[2] / scalar;
	...
	float MathF_SSE3::Vector4MagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
		return _mm_sqrt_ps(_mm_hadd_ps(_mm_hadd_ps(sqReg, sqReg), _mm_setzero_ps())).m128_f32[0];
	...
	float MathF_SSE3::Vector4SqMagnitudeImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
		return _mm_hadd_ps(_mm_hadd_ps(sqReg, sqReg), _mm_setzero_ps()).m128_f32[0];
	...
	double MathF_SSE3::Vector4MagnitudeImpl(double* vec)
	{
		double xSq = vec[0] * vec[0];
		double ySq = vec[1] * vec[1];
		double zSq = vec[2] * vec[2];
	...
	double MathF_SSE3::Vector4SqMagnitudeImpl(double* vec)
	{
		double xSq = vec[0] * vec[0];
		double ySq = vec[1] * vec[1];
		double zSq = vec[2] * vec[2];
	...
	float MathF_SSE3::Vector4MagnitudeImpl(int* vec)
	{
		int xSq = vec[0] * vec[0];
		int ySq = vec[1] * vec[1];
		int zSq = vec[2] * vec[2];
	...
	float MathF_SSE3::Vector4SqMagnitudeImpl(int* vec)
	{
		int xSq = vec[0] * vec[0];
		int ySq = vec[1] * vec[1];
		int zSq = vec[2] * vec[2];
	...
	void MathF_SSE3::Vector4NormaliseImpl(float* vec)
	{
		__m128 vecReg = _mm_loadu_ps(vec);
		__m128 sqReg = _mm_mul_ps(vecReg, vecReg);
		_mm_storeu_ps(vec, _mm_div_ps(vecReg, _mm_set1_ps(_mm_sqrt_ps(_mm_hadd_ps(_mm_hadd_ps(sqReg, sqReg), _mm_setzero_ps())).m128_f32[0])));
	...
	void MathF_SSE3::Vector4NormaliseImpl(double* vec)
	{
		double xSq = vec[0] * vec[0];
		double ySq = vec[1] * vec[1];
		double zSq = vec[2] * vec[2];
	...
	void MathF_SSE3::ResetMatrix4x4Impl(float* matrix)
	{
	...
	void MathF_SSE3::SetToIdentityImpl(float* matrix)
	{
		_mm_storeu_ps(matrix, _mm_setzero_ps());
		_mm_storeu_ps(matrix + 4, _mm_setzero_ps());
		_mm_storeu_ps(matrix + 8, _mm_setzero_ps());
	...
	void MathF_SSE3::SetToScaleImpl(float* matrix, float* vec2)
	{
		_mm_storeu_ps(matrix, _mm_setzero_ps());
		_mm_storeu_ps(matrix + 4, _mm_setzero_ps());
		_mm_storeu_ps(matrix + 8, _mm_setzero_ps());
	...
	void MathF_SSE3::SetToRotationImpl(float* matrix, float radians)
	{
		_mm_storeu_ps(matrix, _mm_setzero_ps());
		_mm_storeu_ps(matrix + 4, _mm_setzero_ps());
		_mm_storeu_ps(matrix + 8, _mm_setzero_ps());
	...
	void MathF_SSE3::SetToScaleWithoutResetImpl(float* matrix, float* vec2)
	{
		matrix[0] = vec2[0];
		matrix[5] = vec2[1];
		matrix[10] = 1.0f;
	...
	void MathF_SSE3::SetToRotationWithoutResetImpl(float* matrix, float radians)
	{
		float sinAngle = std::sin(radians);
		float cosAngle = std::cos(radians);
		matrix[0] = cosAngle;
