...
namespace EmberEngine
{
	...
	float MathF_SSE41::Vector3DotImpl(float* vec1, float* vec2)
	{
		__m128 reg = _mm_mul_ps(_mm_loadu_ps(vec1), _mm_loadu_ps(vec2));
		return reg.m128_f32[0] + reg.m128_f32[1] + reg.m128_f32[2];
	}

	double MathF_SSE41::Vector3DotImpl(double* vec1, double* vec2)
	{
	...
	void MathF_SSE41::Vector3CrossImpl(float* dst, float* vec1, float* vec2)
	{
	...
	void MathF_SSE41::Vector3CrossImpl(double* dst, double* vec1, double* vec2)
	{
	...
	void MathF_SSE41::MultiplyMatrix4x4Impl(float* dst, float* matrix1, float* matrix2)
	{
